...
//		ee_printf("Switching SDRAM to software control.\n");
	}
...
//		ee_printf("Switching SDRAM to hardware control.\n");
	}
#if CSR_DDRPHY_EN_VTC_ADDR
...
static void sdram_leveling_center_module(
	...
	if (show_long)
#ifdef SDRAM_DELAY_PER_DQ
//		ee_printf("m%d dq_line:%d: |", module, dq_line);
#else
//		ee_printf("m%d: |", module);
#endif // SDRAM_DELAY_PER_DQ
...
//			ee_printf("delays: %02d+-%02d", delay_mid, delay_range);
	}
...
void sdram_write_leveling_force_cmd_delay(int taps, int show) {
	...
	if (show)
//		ee_printf("Forcing Cmd delay to %d taps\n", taps);
...
#ifdef SDRAM_WRITE_LEVELING_CMD_DELAY_DEBUG
//		ee_printf("Delay mean: %d/256, ideal: %d/256\n", delay_mean, ideal_delay);
#else
//		ee_printf("%d", ok);
#endif // SDRAM_WRITE_LEVELING_CMD_DELAY_DEBUG
	}
...
//	ee_printf("  tCK equivalent taps: %d\n", _sdram_tck_taps);
...
//	ee_printf("  Setting Cmd/Clk delay to %d taps.\n", best_cdly);
...
static unsigned int sdram_read_leveling_scan_module(int module, int bitslip, int show, int dq_line) {
	...
	score = 0;
	if (show)
//		ee_printf("  m%d, b%02d: |", module, bitslip);
...
#ifdef SDRAM_DELAY_PER_DQ
//			ee_printf("  best: m%d, b%02d, dq_line%d ", module, best_bitslip, dq_line);
#else
//			ee_printf("  best: m%d, b%02d ", module, best_bitslip);
#endif // SDRAM_DELAY_PER_DQ
			sdram_leveling_action(module, dq_line, read_rst_dq_bitslip);
...
#ifdef SDRAM_DELAY_PER_DQ
//				ee_printf("m%d dq%d:%d ", module, dq_line, bitslip);
#else
//				ee_printf("m%d:%d ", module, bitslip);
#endif // SDRAM_DELAY_PER_DQ
...
#ifdef SDRAM_PHY_WRITE_LEVELING_CAPABLE
//	ee_printf("Write leveling:\n");
	sdram_write_leveling();
#endif // SDRAM_PHY_WRITE_LEVELING_CAPABLE

#ifdef SDRAM_PHY_WRITE_LATENCY_CALIBRATION_CAPABLE
//	ee_printf("Write latency calibration:\n");
	sdram_write_latency_calibration();
#endif // SDRAM_PHY_WRITE_LATENCY_CALIBRATION_CAPABLE

#ifdef SDRAM_PHY_WRITE_DQ_DQS_TRAINING_CAPABLE
//	ee_printf("Write DQ-DQS training:\n");
	sdram_write_dq_dqs_training();
#endif // SDRAM_PHY_WRITE_DQ_DQS_TRAINING_CAPABLE

#ifdef SDRAM_PHY_READ_LEVELING_CAPABLE
//	ee_printf("Read leveling:\n");
	sdram_read_leveling();
#endif // SDRAM_PHY_READ_LEVELING_CAPABLE
...
//	ee_printf("Initializing SDRAM @0x%08lx...\n", MAIN_RAM_BASE);
	sdram_software_control_on();
...
static void sdram_debug_error_stats(void) {
	ee_printf("Running initial memtest to fill memory ...\n");
	...
	ee_printf("Running read-only memtests ... \n");
	for (int i = 0; i < SDRAM_DEBUG_STATS_NUM_RUNS; ++i) {
		ee_printf("Running read-only memtest %3d/%3d ... \r", i + 1, SDRAM_DEBUG_STATS_NUM_RUNS);
...
static void sdram_debug_readback(void) {
	ee_printf("Using storage @0x%08x with size 0x%08x for %d readbacks.\n",
	...
	for (int i = 0; i < SDRAM_DEBUG_READBACK_COUNT; ++i) {
		...
		ee_printf("Running readback %3d/%3d ... \r", i + 1, SDRAM_DEBUG_READBACK_COUNT);
